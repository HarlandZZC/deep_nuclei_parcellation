...
def compute_silhouette_coefficient(folder):
    ...
    for subdir in os.listdir(folder):
        if subdir.startswith('sub-'):
            ...
            if os.path.exists(file_path):
                ...
                score18 = silhouette_score(coordinates[idx18], labels[idx18])
                print(f'Silhouette Coefficient for {subdir} label 18: {score18}')
                score54 = silhouette_score(coordinates[idx54], labels[idx54])
                print(f'Silhouette Coefficient for {subdir} label 54: {score54}')
    ...
    with open(os.path.join(args.infolder, 'S.csv'), 'w') as f:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Compute S")
